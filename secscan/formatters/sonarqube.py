...
from secscan.formatters.json import json_output


def get_sonarqube_issue(secscan_issue):
    ...
    secondary_locations = []
    issue_data = secscan_issue['metadata']
    # Handle missing controls
    if not secscan_issue.get('files'):
        primary_location = None
    else:
        for ix, file in enumerate(secscan_issue['files']):
            text_range = {
