...
import secscan.settings as config
from secscan.logger import init_logger
...
def get_config(base_path, config_file):
    ...
    if config_file:
        ...
        cfile = Path(base_path[0]) / config.SECSCAN_CONFIG_FILE
    if cfile.is_file() and cfile.exists():
        extras = read_yaml(cfile)
        root = validate_config(extras, options)
        if not root:
            logger.warning('Invalid YAML, ignoring config from .secscan')
            return options
